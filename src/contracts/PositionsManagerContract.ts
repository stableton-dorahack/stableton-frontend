...
function initPositionsManagerContract_init_args(
  src: PositionsManagerContract_init_args
) {
  return (builder: Builder) => {
    let b_0 = builder;
  };
...
export class PositionsManagerContract implements Contract {
  ...
  async send(
    ...
    if (
      ...
      typeof message === 'object' &&
      !(message instanceof Slice) &&
      message.$$type === 'DepositCollateralMessage'
    ) {
      body = beginCell()
        .store(storeDepositCollateralMessage(message))
        .endCell();
    }
    if (
      message &&
      typeof message === 'object' &&
      !(message instanceof Slice) &&
      message.$$type === 'NewPositionIdMessage'
    ) {
    ...
    if (
      ...
      typeof message === 'object' &&
      !(message instanceof Slice) &&
      message.$$type === 'WithdrawStablecoinMessage'
    ) {
      body = beginCell()
        .store(storeWithdrawStablecoinMessage(message))
        .endCell();
    }
    if (
      message &&
    ...
    if (
      ...
      typeof message === 'object' &&
      !(message instanceof Slice) &&
      message.$$type === 'RepayStablecoinMessage'
    ) {
    ...
    if (
      ...
      typeof message === 'object' &&
      !(message instanceof Slice) &&
      message.$$type === 'WithdrawCollateralMessage'
    ) {
      body = beginCell()
        .store(storeWithdrawCollateralMessage(message))
        .endCell();
    }
    if (
      message &&
  ...
  async getUserPositionAddress(provider: ContractProvider, user: Address) {
    ...
    let source = (await provider.get('userPositionAddress', builder.build()))
      .stack;
  ...
  async getUserPositionAddressById(provider: ContractProvider, id: bigint) {
    ...
    let source = (
      await provider.get('userPositionAddressById', builder.build())
    ).stack;
    let result = source.readAddress();
    return result;
  }
